...
let shardingService = new ShardingService<Item>(3, 'items', {items: 'id,userId,postId'});

beforeEach(async () => {
  // Clears the database before each test.
  shardingService=new ShardingService<Item>(3, 'items', {items: 'id,userId,postId'});
...
describe('ShardingService', () => {
  describe('insertItem', () => {
    it('should insert an item into the database', async () => {
      ...
      const item_response = await shardingService.getItemById('1', 'items');
      // Assert
  ...
  describe('getItemById', () => {
    it('should return null when getting a non-existent item', async () => {
      // Arrange
      // Act
      const item_response = await shardingService.getItemById('999', 'items');
      // Assert
  ...
  describe('insertMultipleItems', () => {
    it('should insert multiple items into the database', async () => {
      ...
      // Assert
      for (const item of items) {
        const item_response = await shardingService.getItemById(item.id, 'items');
        expect(item_response).toEqual(item);
      }
    });
  });
  ...
  describe('putItemBatch', () => {
    it('should insert 30000 items into the database using insertItemBatch', async () => {
      ...
      // Assert
      const item_response = (await shardingService.getAllItems('items')).sort((a, b) => a.id.localeCompare(b.id));
      for (const item of items) {
